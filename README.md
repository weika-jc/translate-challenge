...
当前 `haiku-4-5-opt` 方案对测试 Prompt `7ZJL56AKIU` 显式启用 Structured Outputs，Schema 为 `{"c": string}`。Schema 被拒绝时不会自动降级；输出不可解析时会按统一策略重试，并通过首次输出可用率保留格式能力差异。后续 GPT-5.6 Luna 的 Bedrock 方案应将该开关保持为 `False`，只通过 Prompt 约束输出格式。
